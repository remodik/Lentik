...
import type { FamilyDetail, MyFamily } from "../api/types";

const roleLabel = (role: string): string => (role === "owner" ? "Владелец" : "Участник");
...
export default function ProfileScreen() {
  ...
  const [allFamilies, setAllFamilies] = useState<MyFamily[]>([]);
  ...
  const handleSwitchFamily = (fam: MyFamily) => {
    void selectFamily(fam);
            ...
            {loading ? (
              ...
              <View style={styles.membersList}>
                <Text style={styles.membersTitle}>
                  Участники ({familyDetail.members.length})
                </Text>
                {familyDetail.members.map((m) => (
                  <View key={m.user_id} style={styles.memberRow}>
                    ...
                    <View
                      style={[
                        styles.onlineDot,
                        { backgroundColor: m.is_online ? colors.online : colors.offline },
                      ]}
                    />
                  </View>
                ))}
      ...
      <Modal visible={showFamilyModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            {allFamilies.map((fam) => {
              const active = fam.family_id === currentFamily?.family_id;
              return (
                <TouchableOpacity
                  key={fam.family_id}
                  style={[styles.famRow, active && styles.famRowActive]}
                ...
                >
                  ...
                  />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.famName, active && { color: colors.white }]}>
                      {fam.family_name}
                    </Text>
                    <Text style={[styles.famRole, active && { color: colors.primaryLight }]}>
                      {roleLabel(fam.role)}
                    </Text>
                  </View>
                  {active && (
