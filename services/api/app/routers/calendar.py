...
async def update_event(
    ...
    if "title" in updated_fields and body.title is not None:
        event.title = body.title
    if "description" in updated_fields:
        event.description = body.description
    if "starts_at" in updated_fields and body.starts_at is not None:
        if event.starts_at != body.starts_at:
            reset_reminder = True
        event.starts_at = body.starts_at
    if "ends_at" in updated_fields:
        event.ends_at = body.ends_at
    if "color" in updated_fields and body.color is not None:
        event.color = body.color
    if "reminder_minutes" in updated_fields:
        if event.reminder_minutes != body.reminder_minutes:
            reset_reminder = True
