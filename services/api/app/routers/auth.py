...
async def register(
    ...
    """Обычная регистрация — имя + PIN. Семью создаёт отдельно."""
    existing = await db.scalar(select(User).where(User.username == body.username))
    if existing:
        raise HTTPException(status_code=409, detail="Имя уже занято")

    user = User(username=body.username, password_hash=hash_pin(body.pin))
    db.add(user)
    await db.flush()
...
async def join_by_invite(
    ...
    user = User(username=username, password_hash=hash_pin(body.pin))
    db.add(user)
    await db.flush()

    membership = Membership(user_id=user.id, family_id=family_id, role=Role.MEMBER)
    db.add(membership)
