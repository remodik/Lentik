...
type ShareInput = { user_id: string; share: string };
...
const labelFor = (cat: string): string => CATEGORY_LABELS[cat] || cat;
const iconFor = (cat: string, type: BudgetType): IconName =>
  CATEGORY_ICONS[cat] || (type === "income" ? "trending-up-outline" : "pricetag-outline");

const formatAmount = (val: Money): string => {
...
const splitEqually = (amount: number, userIds: string[]): ShareInput[] => {
...
export default function BudgetScreen() {
  ...
  const [items, setItems] = useState<BudgetTransaction[]>([]);
  const [summary, setSummary] = useState<BudgetSummary | null>(null);
  const [categories, setCategories] = useState<BudgetCategories>({ income: [], expense: [] });
  const [members, setMembers] = useState<FamilyMember[]>([]);
  const [balances, setBalances] = useState<BudgetMemberBalance[]>([]);
  ...
  const [formParticipants, setFormParticipants] = useState<Record<string, boolean>>({});
  const [formCustomShares, setFormCustomShares] = useState<Record<string, string>>({});
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const filterType: BudgetType | undefined = filter === "all" ? undefined : filter;
  ...
  const buildSplitsPayload = (amountNum: number): ShareInput[] | null => {
  ...
  const handleSave = async () => {
    ...
    let splits: ShareInput[] | null = null;
    if (formShared) {
      ...
      if (Math.abs(sum - amount) > 0.01) {
        Alert.alert(
          "Ошибка",
          `Сумма долей (${formatAmount(sum)}) должна равняться ${formatAmount(amount)}`,
        );
        return;
      }
      if (!formPaidBy) {
    ...
    setSaving(true);
    try {
      const payload: BudgetTransactionCreateInput = {
        type: formType,
  ...
  const renderTxItem = ({ item }: { item: BudgetTransaction }) => {
  ...
  const equalShareHint = useMemo<ShareInput[] | null>(() => {
    const amount = parseFloat(formAmount.replace(",", "."));
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.dateRow}>
                <TouchableOpacity
                  ...
                  onPress={() => setFormDate(shiftDate(formDate, -1))}
                  activeOpacity={0.7}
                >
                ...
                <View style={styles.sharedSection}>
                  <View style={styles.sharedToggle}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.sharedHint}>Учтётся в балансах семьи</Text>
                    </View>
                    <Switch
                      value={formShared}
                        ...
                        return (
                          <View key={m.user_id} style={styles.participantRow}>
                            <Switch
                              value={on}
                              onValueChange={(v) =>
                                setFormParticipants((p) => ({ ...p, [m.user_id]: v }))
                              }
                              trackColor={{ false: colors.border, true: colors.primaryLight }}
                              thumbColor={on ? colors.primary : colors.textMuted}
                            />
                            <Text
                              style={[styles.participantName, !on && { color: colors.textMuted }]}
                            >
                              {m.display_name}
                            </Text>
                            {on && !formEqual ? (
                              <TextInput
                                ...
                                onChangeText={(v) =>
                                  setFormCustomShares((p) => ({ ...p, [m.user_id]: v }))
                                }
...
const styles = StyleSheet.create({
  ...
  sharedToggle: { flexDirection: "row", alignItems: "center", gap: spacing.md },
