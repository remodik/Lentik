...
export default function RolesEditor({ familyId, isOwner }: Props) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setRoles((prev) => [...prev, next].sort((a, b) => a.priority - b.priority));
  ...
  return (
    <div className="flex gap-5 h-full min-h-[420px]">
      {/* Список ролей */}
      <div className="w-[240px] shrink-0 flex flex-col border border-[color:var(--border-glass)] rounded-2xl bg-[color:var(--bg-surface-subtle)]">
        ...
        <ul className="flex-1 overflow-y-auto sidebar-scroll px-1.5 py-1.5 space-y-0.5 rounded-b-2xl">
          {roles.map((r) => {
