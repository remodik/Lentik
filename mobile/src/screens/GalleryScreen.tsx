...
const buildUrl = (url: string | null | undefined): string | undefined => {
...
export default function GalleryScreen() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await uploadToGallery(currentFamily.family_id, asset.uri, fileName, mimeType);
      await loadGallery();
    } catch (err) {
      Alert.alert("Ошибка", apiErrorMessage(err, "Ошибка загрузки"));
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        >
          <Ionicons
            name={uploading ? "hourglass-outline" : "add-circle"}
            size={30}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Image source={{ uri: buildUrl(item.url) }} style={styles.thumb} resizeMode="cover" />
